...
struct Core {
    static func main(out: URL, in files: [String], publicMemberwiseInit: Bool) throws {
        ...
        let typesSwift: [String: (service: String, structs: String, extensions: String)] = {
            ...
            types.forEach { type in
                ...
                let structs = type.type.swift(
                    types.map { $0.type }, prefix: prefix,
                    publicMemberwiseInit: publicMemberwiseInit)
                let extensions = type.type.dictionariesForExpressibleByXMLProtocol(
                    types.map { $0.type }, typeQualifier: []
                ).map {
        ...
        try typesSwift.forEach { prefix, swifts in
            ...
            let file = out.deletingLastPathComponent().appendingPathComponent(
                "WSDL+\(purePrefix).swift")
    ...
    fileprivate static func parseXSD(_ path: String, prefix: String) -> [(
        prefix: String, type: XSDType
    )]? {
        var options = AEXMLOptions()
        ...
        guard
            let xsd = try? AEXMLDocument(
                xml: Data(contentsOf: URL(fileURLWithPath: path)), options: options)
        else { return nil }
        return parseXSD(xsd, prefix: prefix)
    }

    fileprivate static func parseXSD(_ xsd: AEXMLDocument, prefix: String) -> [(
        prefix: String, type: XSDType
    )]? {
...
struct WSDL {
    ...
    func swift() -> String {
        return try! template(named: "WSDLService").render([
            ...
            "path": {
                let p =
                    URL(string: service.port.location)?.path
                    ?? (service.port.location as NSString).lastPathComponent
                return String(p.dropFirst(p.first == "/" ? 1 : 0))
            }(),
            "operations": portType.operations.map { op -> [String: String] in
                let inputMessage = messages.first {
                    $0.name == replaceTargetNameSpace(op.inputMessage, prefix: "")
                }!
                let outputMessage = messages.first {
                    $0.name == replaceTargetNameSpace(op.outputMessage, prefix: "")
                }!
                return [
...
struct WSDLPortType {
    ...
    static func deserialize(_ node: AEXMLElement) -> WSDLPortType? {
        guard let name = node.attributes["name"],
            let operations = node["operation"].all?.flatMap({ WSDLOperation.deserialize($0) })
        else {
            NSLog("%@", "cannot deserialize \(self) from node \(node.xmlCompact)")
            return nil
        }
...
struct XSDType {
    ...
    static func deserialize(_ node: AEXMLElement, prefix: String = "") -> XSDType? {
        func resolveName() -> String? {
            ...
            if let name = node.parent?.attributes["name"], node.parent?.name == "element" {
                return name
            }
            return nil
        }
        ...
        func parseChildElements(_ sequence: AEXMLElement) {
            func definedByRootElement(_ parent: AEXMLElement?) -> Bool {
                ...
                // root is <schema>
                if parent.parent?.name == "schema" && parent.name == "element" {
                    return true
                }
            ...
            for sn in sequence.children {
                switch sn.name {
                case "element":
                    if let e = XSDElement.deserialize(
                        sn, prefix: prefix, definedByRootElement: byRoot)
                    {
                        elements.append(e)
                    }
                default:
        ...
        return self.init(
            prefix: prefix, bareName: name, elements: elements, base: base.map { prefix + $0 })
    }
    ...
    func dictionary(
        _ env: [XSDType], prefix: String, publicMemberwiseInit: Bool, typeQualifier: [String] = []
    ) -> [String: Any] {
        let baseType = self.baseType(env)
        let elements = self.elements.map { $0.dictionary(prefix) }
        let bases = baseType.map {
            $0.dictionary(env, prefix: prefix, publicMemberwiseInit: publicMemberwiseInit)
        }
        ...
        return [
            ...
            "base": bases ?? [:],
            "publicMemberwiseInit": publicMemberwiseInit,
            "xmlParams": (self.elements + (baseType?.elements ?? [])).map {
                ["name": $0.name, "swiftName": $0.swiftName, "xmlns": $0.xmlns]
            },
            "innerTypes": self.elements.flatMap { e -> String? in
                if case .inner(let t) = e.type {
                    return t.swift(
                        env, prefix: prefix, publicMemberwiseInit: publicMemberwiseInit,
                        typeQualifier: typeQualifier + [name])
                } else {
                    return nil
                }
            },
    ...
    func dictionariesForExpressibleByXMLProtocol(_ env: [XSDType], typeQualifier: [String] = [])
        -> [[String: Any]]
    {
        var ds: [[String: Any]] = []
        ...
        ds.append([
            ...
            "xmlParams": (self.elements + (baseType(env)?.elements ?? [])).map {
                ["name": $0.name, "swiftName": $0.swiftName, "xmlns": $0.xmlns]
            },
        ])
        for case .inner(let t) in (elements.map { $0.type }) {
            ds.append(
                contentsOf: t.dictionariesForExpressibleByXMLProtocol(env, typeQualifier: fqn))
        }
        return ds
    }
...
struct XSDElement {
    ...
    static func deserialize(_ node: AEXMLElement, prefix: String = "", definedByRootElement: Bool)
        -> XSDElement?
    {
        guard let name = node.attributes["name"] else {
            NSLog("%@", "cannot deserialize \(self) from node \(node.xmlCompact)")
